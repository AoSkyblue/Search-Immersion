...
function initNestHub() {
        ...
        <div class="st-main">
          ...
          <div class="st-content-scroll">
            ...
            <div id="tab-general" class="st-section active">
              ...
              <div class="st-row">
                <span>${t('search_shortcut_label')}</span>
                <input type="text" id="set-search-shortcut" class="st-input" style="width:80px; text-align:center;" maxlength="1" placeholder="/">
              </div>
  ...
  startClock();
  // 検索機能を動的importで読み込み（ESモジュール）
  (async () => {
    try {
      const { setupSearch } = await import(chrome.runtime.getURL('components/search.js'));
      setupSearch(t);
    } catch (e) {
      console.error('Search module load failed:', e);
    }
  })();
  setupMemo();
  setupCountdown();
...
function applyPreferences() {
  ...
  setVal('set-search-shortcut', prefs.searchShortcut || '/');
...
function savePreferences() {
  ...
  const prefs = {
    ...
    searchShortcut: getVal('set-search-shortcut') || '/'
  };
